...
while true do
    ...
    counter = counter + 1
    if counter > 6 then
        fail(string.format('Failed to load game dependencies. lol. Details: %s, %s, %s', type(framework), typeof(scrollHandler), type(network)))
    end
    wait(1)
end
...
UI.AccentColor = Color3.fromRGB(0, 176, 242)
UI.AccentColorDark = UI:GetDarkerColor(UI.AccentColor);
UI:UpdateColorsUsingRegistry()

local Window = UI:CreateWindow({
    Title = string.format('WitterHub AutoPlayer - Version 1.1 | Updated: %s', metadata.version, metadata.updated),
    AutoShow = true,
...
Groups.Credits = Tabs.Main:AddRightGroupbox('Credits')
    Groups.Credits:AddLabel('<font color="#FF0000">witter</font> - script')
    Groups.Credits:AddLabel('<font color="#FF8300">PornHub</font> - contributor')
...
Groups.Misc = Tabs.Main:AddRightGroupbox('Miscellaneous')
    ...
    Groups.Misc:AddButton('Copy discord', function()
        if pcall(setclipboard, "https://discord.gg/pornhub") then
            UI:Notify('Successfully copied discord link to your clipboard!', 5)
        end
    end)

    Groups.Misc:AddLabel('Menu toggle key'):AddKeyPicker('MenuToggle', { Default = 'Insert', NoUI = true })
...
game:GetService("StarterGui"):SetCore("SendNotification",{     
    Title = "Important!",     
    Text = "WitterHub Own u",
    Button1 = "STFU",Button2 = "Fuck LGBTQ+",     Duration = 20, })